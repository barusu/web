<template>
  <div class="puzzle-wrapper side">
    <div class="preview">
      <div class="content">
        <o-chart class="components" :option="op"></o-chart>
      </div>
    </div>
    <div class="config">
      <div class="tabs">
      ...
      <div class="swiper-container" id="pie_swiper">
        <div class="swiper-wrapper">
          <div class="swiper-slide form">
            ...
            <div class="group" v-for="(s, index) in op.series">
              <p>
                <o-input :label="'系列' + (index + 1) + '名称'" placeholder="请输入系列名称" v-model="s.name" type="single"></o-input>
              </p>
              <p>
                <o-select :label="'系列' + (index + 1) + '数据项'" placeholder="请选择显示数据的属性" type="single" :data="keys" v-model="s.value"></o-select>
              </p>
              <span class="icon" :class="index === 0 ? '' : 'error'">
                <o-svg :type="index === 0 ? 'plus' : 'del'" @click="operSeries(index)"></o-svg>
              </span>
            </div>
          </div>
...
<script>
  import oChart from '@/components/ui/chart-bar';
  ...
  export default {
    ...
    watch: {
      op: {
        deep: true,
        handler() {
          if(this.op.url && this.op.dataX && this.op.series.length) this.$emit('update', this.op);
          console.log('up');
        }
      },
      tabIndex() {
    ...
    methods: {
      operSeries(index) {
        if(index) {
          this.op.series.splice(index, 1);
        }else {
          this.op.series.push({name: '', value: ''});
        }
      },
      updateBar(index) {
    ...
    mounted() {
      ...
      if(this.option.series && Array.isArray(this.option.series)) this.op.series = this.option.series;
      else this.op.series = [];
      if(!this.op.series.length) {
        this.op.series.push({name: '', value: ''});
      }
